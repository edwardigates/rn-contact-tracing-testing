...
public final class EventDao_Impl implements EventDao {
  ...
  public EventDao_Impl(RoomDatabase __db) {
    ...
    this.__insertionAdapterOfEvent = new EntityInsertionAdapter<Event>(__db) {
      ...
      public String createQuery() {
        return "INSERT OR ABORT INTO `events` (`id`,`timestamp`,`device_name`,`action_type`,`success`,`errorMessage`) VALUES (nullif(?, 0),?,?,?,?,?)";
      }
    ...
    this.__updateAdapterOfEvent = new EntityDeletionOrUpdateAdapter<Event>(__db) {
      @Override
      public String createQuery() {
        return "UPDATE OR ABORT `events` SET `id` = ?,`timestamp` = ?,`device_name` = ?,`action_type` = ?,`success` = ?,`errorMessage` = ? WHERE `id` = ?";
      }
  ...
  public List<Event> getAllEvents() {
    ...
    try {
      ...
      while(_cursor.moveToNext()) {
        ...
        _item = new Event(_tmpTimestamp,_tmpDeviceName,_tmpActionType,_tmpSuccess,_tmpErrorMessage);
        final int _tmpId;
  ...
  public List<Event> getEventsByActionType(final String actionType) {
    final String _sql = "SELECT * FROM events WHERE action_type = ? ORDER BY device_name,timestamp desc";
